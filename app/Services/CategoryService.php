...
class CategoryService
{
    ...
    public static function generateGroupKey($title)
    {
        ...
        // Группировка по кавычкам (если в названии есть "...")
        if (preg_match('/^(.*?"[^"]+")/u', $title, $matches)) {
            return trim($matches[1]);
        }

        // 1. Если в названии есть запятая — берём всё, что до первой запятой.
        if (strpos($title, ',') !== false) {
            return trim(explode(',', $title)[0]);
        }

        // 2. Если в названии встречается ; — берём всё, что до первой точки с запятой.
        if (strpos($title, ';') !== false) {
            return trim(explode(';', $title)[0]);
        }

        // 3. Если название начинается с четырёх цифр, то группируем по ним.
        if (preg_match('/^(\d{4})/', $title, $matches)) {
            return $matches[1];
        }

        // 4. Если встречается шаблон , размер M/L/XL/..., берём часть до запятой и слова "размер".
        if (preg_match('/(.*?)\s*,\s*размер\s*[SMLX]+/ui', $title, $matches)) {
            return trim($matches[1]);
        }

        // 5, Если написано без запятой (размер M/L/XL), берём часть до слова "размер".
        ...
        // 3а. Группировка по подчёркиванию + размер (например: _M, _XL, _2XL)
        if (preg_match('/^(.*?)(?:_[SMLX\d]+)$/ui', $title, $matches)) {
            return trim($matches[1]);
        }
    ...
    protected static function isColorWord($word)
    {
        ...
        $colors = ['красный', 'красная', 'синий', 'тёмно-синий', 'зеленый',  'ярко-зеленый', 'черный', 'белый', 'желтый', 'жёлтый',
